...
!---^^^^^^^^^
    ...
    !*      return pointers to phi' and dphi' which can be used to generate state and derivative at z
        ...
        subroutine update0( this,step,dz )
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    !*          k4 = f( phi(z) + dz k3 )
            ...
            integer,intent(out),optional                        ::      dz

            if (present(dz)) dz = 0

            if (.not. present(step)) then
                !   initialising. Have set phip = phi0.
                this%phi0(:) = this%phip(:)
                !this%phiz(:) = this%phi0(:)
                return
            end if

            !print *,"Lib_RK4::update0 info - step ",step
            !print *,"Lib_RK4::update0 info - minmaxavg phi0  ",minval(abs(this%phi0)),maxval(abs(this%phi0)),sum(abs(this%phi0))/this%n
            !print *,"Lib_RK4::update0 info - minmaxavg phi'  ",minval(abs(this%phip)),maxval(abs(this%phip)),sum(abs(this%phip))/this%n
            !print *,"Lib_RK4::update0 info - minmaxavg phiz  ",minval(abs(this%phiz)),maxval(abs(this%phiz)),sum(abs(this%phiz))/this%n
            !print *,"Lib_RK4::update0 info - minmaxavg dphi' ",minval(abs(this%dphip)),maxval(abs(this%dphip)),sum(abs(this%dphip))/this%n
