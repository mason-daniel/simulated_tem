            ...
            do
                ...
                select case(lt)
                    case (NBAX_OPEN:NBAX_OPENCLOSE)
                        ...
                        if (lt == NBAX_OPENCLOSE) then 
                            !   can't use "/" as a tokenizer, as it could be part of a filename. But <helloworld/> can't parse to "helloworld/"
                            uuu = len_trim(sss)
                            if (sss(uuu:uuu)=="/") sss=sss(1:uuu-1)
                        end if
                        if (opened) then
