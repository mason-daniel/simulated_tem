...
!---^^^^^^^^
        ...
        subroutine readExtinctionDistance( filename,hkl,symm,xi_g, ok )
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    !*      attempt to read in xi_g for reflection [hkl]
        ...
        !---    not all hkl reflections will be listed.
            ...
            !---    first two lines of file are expected to be comments
                ...
                read(unit=850,fmt='(a)',iostat=ioerr) dummy
                if (ioerr/=0) then
                    print *,"Lib_ReadExtinctionDistances::readExtinctionDistance error - can't read header """//trim(filename)//""""
                    return
                end if
            ...
            !---    find the line we are looking for.
                do ii = 1,1000      !   don't search forever... if the file is too long, its bound to be an error!
                    ...
                    if (ioerr/=0) then
                        print *,"Lib_ReadExtinctionDistances::readExtinctionDistance error - end of file reached/incompatible format """//trim(filename)//""""
                        exit
                    end if
            ...
            if (.not. ok) then
                do jj = 1,nReflection
                    if (.not. found(jj)) write(*,fmt='(a,3i4,a)') " Lib_ReadExtinctionDistances::readExtinctionDistance warning - reflection ",hkl(:,jj)," not found"
                end do
            end if
            ...
            return            
        end subroutine readExtinctionDistance                                                                                             
