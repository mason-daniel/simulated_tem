! ******************************************************************************
! 
!                   FORTRAN SOURCE FILE       inputREAL.f
!
! ******************************************************************************** 
!
!                                                        by X. L. Dudarev, 
...
!
!  X L Dudarev and M J Whelan, Surface Science 310 (1994) 373 
!  X L Dudarev, D D Vvedensky and M J Whelan, Phys. Review B50 (1994) 14525 
!  X L Dudarev, L M Peng and M J Whelan, Surface Science 330 (1995) 86
!  see also
!  L M Peng, X L Dudarev and M J Whelan, High-Energy Electron Diffraction and Microscopy
...
!
!   Edit by DRM 17/09/24
!       refactored as f08 compatible module
!       removed obsolete implicit statements
!       replaced file writing with return of data 

    module Lib_inputReal
!---^^^^^^^^^^^^^^^^^^^^
!*      
        ...
        character(len=8),private,parameter      ::      VERSION = "0.0.1"
        real(kind=real64),public,parameter     ::      LIB_INPUTREAL_SMIN = 0.0d0          !   minimum scattering s = q/4pi
        real(kind=real64),public,parameter     ::      LIB_INPUTREAL_SMAX = 5.0d0          !   maximum scattering s = q/4pi
        integer,public,parameter               ::      LIB_INPUTREAL_NS = 500              !   number of s intervals
        
        real(kind=real64),private,parameter     ::      CF=41.78214D0


 
        public      ::      scatteringAmplitude     !   calls FLI,FBE etc according to element
        public      ::      getScatteringAmplitude

              
        public      ::      FLI,FBE,FC ,FO ,FNA,FMG,FAL,FSI
        public      ::      FK ,FCA,FTI,FV ,FCR,FMN,FFE,FCO
        public      ::      FNI,FCU,FZN,FGA,FGE,FAS,FRB,FSR
        public      ::      FY ,FZR,FNB,FMO,FRU,FRH,FPD,FAG
        public      ::      FCD,FSN,FCS,FBA,FLA,FCE,FGD,FHF
        public      ::      FTA,FW ,FRE,FOS,FIR,FPT,FAU,FTL
        public      ::      FPB,FTH

        interface   scatteringAmplitude
            module procedure scatteringAmplitude0
            module procedure scatteringAmplitude1
        end interface
...
!---^^^^^^^^
        ...
        pure real(kind=real64) function getSmin()
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            getSmin = LIB_INPUTREAL_SMIN
            return
        end function getSmin


        pure real(kind=real64) function getSmax()
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            getSmax = LIB_INPUTREAL_SMAX
            return
        end function getSmax


        pure integer function getNS()
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            getNS = LIB_INPUTREAL_NS
            return
        end function getNS



        subroutine getScatteringAmplitude(el,f)
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    !*      construct the scattering amplitude for element el
            character(len=*),intent(in)                     ::      el
            real(kind=real64),dimension(0:),intent(out)     ::      f
            integer                     ::      ii,ee
            real(kind=real64)           ::      ss,ds

            ds = ( LIB_INPUTREAL_SMAX-LIB_INPUTREAL_SMIN )/LIB_INPUTREAL_NS
            ee = whichElement(el)

            do ii = 0,LIB_INPUTREAL_NS
                ss = LIB_INPUTREAL_SMIN + ii*ds
                f(ii) = scatteringAmplitude(ee,ss)
            end do

            return
        end subroutine getScatteringAmplitude


        real(kind=real64) function scatteringAmplitude0(el,s)
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    !*      return the scattering amplitude given the element
    !*      and the value s = 1/4pi.    
            character(len=*),intent(in)         ::      el
            real(kind=real64),intent(in)        ::      s
            integer         ::      ii
            ii = whichElement(el)
            if (ii==0) then
                print *,"Lib_inputReal::scatteringAmplitude error - did not recognise input element """//trim(el)//""""
                scatteringAmplitude0 = 0
            else
                scatteringAmplitude0 = scatteringAmplitude1(ii,s)
            end if

            return
        end function scatteringAmplitude0

        real(kind=real64) function scatteringAmplitude1(i,s)
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    !*      return the scattering amplitude given the element indexed i
    !*      and the value s = 1/4pi.    
            integer,intent(in)                  ::      i
            real(kind=real64),intent(in)        ::      s

            select case(i)
                case( 1) ;  scatteringAmplitude1 = FLI(s)
                case( 2) ;  scatteringAmplitude1 = FBE(s)
                case( 3) ;  scatteringAmplitude1 = FC (s)
                case( 4) ;  scatteringAmplitude1 = FO (s)
                case( 5) ;  scatteringAmplitude1 = FNA(s)
                case( 6) ;  scatteringAmplitude1 = FMG(s)
                case( 7) ;  scatteringAmplitude1 = FAL(s)
                case( 8) ;  scatteringAmplitude1 = FSI(s)
                case( 9) ;  scatteringAmplitude1 = FK (s)
                case(10) ;  scatteringAmplitude1 = FCA(s)
                case(11) ;  scatteringAmplitude1 = FTI(s)
                case(12) ;  scatteringAmplitude1 = FV (s)
                case(13) ;  scatteringAmplitude1 = FCR(s)
                case(14) ;  scatteringAmplitude1 = FMN(s)
                case(15) ;  scatteringAmplitude1 = FFE(s)
                case(16) ;  scatteringAmplitude1 = FCO(s)
                case(17) ;  scatteringAmplitude1 = FNI(s)
                case(18) ;  scatteringAmplitude1 = FCU(s)
                case(19) ;  scatteringAmplitude1 = FZN(s)
                case(20) ;  scatteringAmplitude1 = FGA(s)
                case(21) ;  scatteringAmplitude1 = FGE(s)
                case(22) ;  scatteringAmplitude1 = FAS(s)
                case(23) ;  scatteringAmplitude1 = FRB(s)
                case(24) ;  scatteringAmplitude1 = FSR(s)
                case(25) ;  scatteringAmplitude1 = FY (s)
                case(26) ;  scatteringAmplitude1 = FZR(s)
                case(27) ;  scatteringAmplitude1 = FNB(s)
                case(28) ;  scatteringAmplitude1 = FMO(s)
                case(29) ;  scatteringAmplitude1 = FRU(s)
                case(30) ;  scatteringAmplitude1 = FRH(s)    
                case(31) ;  scatteringAmplitude1 = FPD(s)
                case(32) ;  scatteringAmplitude1 = FAG(s)
                case(33) ;  scatteringAmplitude1 = FCD(s)
                case(34) ;  scatteringAmplitude1 = FSN(s)
                case(35) ;  scatteringAmplitude1 = FCS(s)
                case(36) ;  scatteringAmplitude1 = FBA(s)
                case(37) ;  scatteringAmplitude1 = FLA(s)
                case(38) ;  scatteringAmplitude1 = FCE(s)
                case(39) ;  scatteringAmplitude1 = FGD(s)
                case(40) ;  scatteringAmplitude1 = FHF(s)    
                case(41) ;  scatteringAmplitude1 = FTA(s)
                case(42) ;  scatteringAmplitude1 = FW (s)
                case(43) ;  scatteringAmplitude1 = FRE(s)
                case(44) ;  scatteringAmplitude1 = FOS(s)
                case(45) ;  scatteringAmplitude1 = FIR(s)
                case(46) ;  scatteringAmplitude1 = FPT(s)
                case(47) ;  scatteringAmplitude1 = FAU(s)
                case(48) ;  scatteringAmplitude1 = FTL(s)
                case(49) ;  scatteringAmplitude1 = FPB(s)
                case(50) ;  scatteringAmplitude1 = FTH(s)    
                case default
                    print *,"Lib_inputReal::scatteringAmplitude1 error - did not recognise input element index ",i
                    scatteringAmplitude1 = 0
            end select

            return
        end function scatteringAmplitude1

! --------------------------------------------------------------------------

    REAL*8 FUNCTION FAG(X)
! Program for evaluating the scattering amplitude for SILVER atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
    integer                 ::      N
...
! for large arguments the screened Coulomb law is assumed, 
! F proportional to 1./(X**2+X02)
    FAG=(Z/cf)/(X**2+X02)
    RETURN
...
! Program for evaluating the scattering amplitude for ALUMINIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for ARSENIC   &m, inter-
! polation of data from International Tables for X-ray Crysta   &raphy, 1974
! argument X equals to q/4pi within the notations of Landau &   &shits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
    integer                 ::      N
    DIMENSION A(56),XX(56)
...
! Program for evaluating the scattering amplitude for GOLD atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for BARIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for  a BERYLLIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for  CARBON atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for CALCIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for  a CADMIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for CERIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for COBALT atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for CHROMIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for CESIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for a COPPER atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for IRON atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for GALLIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for a GADOLINIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for  SILICON atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
5   FGE=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    ...
    REAL*8 FUNCTION FHF(X)
! Program for evaluation of scattering amplitude for a HAFNIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
    integer                 ::      N
    DIMENSION A(53),XX(53)
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FHF=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
    
    REAL*8 FUNCTION FIR(X)
! Program for evaluation of scattering amplitude for an IRIDIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FIR=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END

    REAL*8 FUNCTION FK(X)
! Program for evaluating the scattering amplitude for  POTASSIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for LANTAN atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for  LITHIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for a MAGNESIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for a MANGANESE atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for MOLIBDENUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for  SODIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
5   FNA=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    ...
    REAL*8 FUNCTION FNB(X)
! Program for evaluation of scattering amplitude for a NIOBIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FNB=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END

    REAL*8 FUNCTION FNI(X)
! Program for evaluating the scattering amplitude for NICKEL atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for an OXIGEN atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
5   FO=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    ...
    REAL*8 FUNCTION FOS(X)
! Program for evaluation of scattering amplitude for a OSMIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FOS=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END

    REAL*8 FUNCTION FPB(X)
! Program for evaluating the scattering amplitude for a LEAD atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
5   FPB=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    ...
    REAL*8 FUNCTION FPD(X)
! Program for evaluation of scattering amplitude for a PALLADIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FPD=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END

    REAL*8 FUNCTION FPT(X)
! Program for evaluation of scattering amplitude for a PLATINUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FPT=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! Program for evaluating the scattering amplitude for RUBIDIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FRE=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! **************************************************************************
    ...
    REAL*8 FUNCTION FRH(X)
! Program for evaluation of scattering amplitude for a RH atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
    integer                 ::      N
    DIMENSION A(52),XX(52)
    ...
    DATA Z/45.D0/
     ...
     ,1.20d0,1.30d0,1.40d0,1.50d0,1.60d0,1.70d0,1.80d0,1.90d0       &
     ,2.00d0/
        X02=Z/(CF*A(52))-XX(52)**2
    IF(X.GT.XX(1)) GO TO 1
    FRH=A(1)
    RETURN
1   IF (X.LT.XX(52)) GO TO 2
! for large arguments the screened Coulomb law is assumed
    FRH=(Z/CF)/(X**2+X02)
    RETURN
2   IF (X.GT.XX(2)) GO TO 21    
        FRH=A(1)-(A(1)-A(2))*X*X/XX(2)**2
        RETURN
 21     N=1
    GO TO 4
3   N=N+1
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FRH=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! **************************************************************************
    ...
    REAL*8 FUNCTION FRU(X)
! Program for evaluation of scattering amplitude for a RUTENIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FRU=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! Program for evaluating the scattering amplitude for a SN atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! Program for evaluating the scattering amplitude for a STRONCIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! **************************************************************************
    ...
    REAL*8 FUNCTION FTA(X)
! Program for evaluation of scattering amplitude for a TALLIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FTA=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! **************************************************************************
    ...
    REAL*8 FUNCTION FTH(X)
! Program for evaluation of scattering amplitude for a THORIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FTH=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! Program for evaluating the scattering amplitude for a TITANIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FTL=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! Program for evaluating the scattering amplitude for VANADIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! **************************************************************************
    ...
    REAL*8 FUNCTION FW(X)
! Program for evaluation of scattering amplitude for a TUNGSTEN atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FW=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! **************************************************************************
    ...
    REAL*8 FUNCTION FY(X)
! Program for evaluation of scattering amplitude for a YTTRIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FY=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
...
! Program for evaluating the scattering amplitude for a ZINK atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/4pi within the notations of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
! **************************************************************************
    ...
    REAL*8 FUNCTION FZR(X)
! Program for evaluation of scattering amplitude for a ZIRCONIUM atom, inter-
! polation of data from International Tables for X-ray Crystallography, 1974
! argument X equals to q/(4*pi) within the notation of Landau & Lifshits
    real(kind=real64),intent(in)       ::      X
    real(kind=real64)       ::      A,Z,XX,X02
...
4   IF (X.LE.XX(N)) GO TO 5
    GO TO 3
5   FZR=A(N-1)+(X-XX(N-1))*(A(N)-A(N-1))/(XX(N)-XX(N-1))
    RETURN
    END
    
    end module Lib_inputReal
