...
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        real(kind=real64),parameter,public      ::      HBAR =  H/(2*3.14159265390d0)         !   Direc constant in eV fs
...
!     
! !   gfortran -ffree-line-length-256 ${MYF90LIB}/Lib_RelativisticElectrons.f90     
!     
!     
...
!         real(kind=real64)       ::      VV
!         
!         print *,"enter accelerating voltage V"
!         read(*,*) VV
!         
!         print *,"wavelength ",wavelength(VV)
!         print *,"velocity   ",velocity(VV)
!         
