...
!---^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        real(kind=real64),dimension(:),allocatable      ::      fimag
        ...
        call getImagScatteringAmplitude(element,V,bdw,fimag)
        ok = .true.
        ...
        do ii = 0,nS
            ...
            write(*,fmt='(a6,i6,3f12.6,l4)') element,ii,ss,fimag(ii) , ff,(len_trim(dummy)>0).or.(abs(ff-fimag(ii))<EPS)
        end do
